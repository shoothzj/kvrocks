...
RUN apk update && apk upgrade && apk --no-cache add git gcc g++ make cmake ninja autoconf automake libtool python3 linux-headers curl openssl-dev libexecinfo-dev redis
WORKDIR /kvrocks
...
RUN apk update && apk upgrade && apk --no-cache add libexecinfo
